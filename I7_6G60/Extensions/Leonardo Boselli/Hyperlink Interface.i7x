Version 2 of Hyperlink Interface (for Glulx only) by Leonardo Boselli begins here.
...
"Heavily based on Version 8 of Keyword Interface by Aaron Reed."
...
Object hyperlink highlighting is a truth state that varies. Object hyperlink highlighting is usually true. [Global activation of object hyperlinks.]
...
First when play begins:
	...
	let T be "exits";
	add T to List of Hyperlink Glulx Replacement Commands;
	[change the command prompt to "[set link 2]look[end link] | [set link 3]inventory[end link] | [set link 1]menu[end link] >"];
...
Rule for direction hyperlinking a direction (this is the Hyperlink Interface direction hyperlinking rule):
...
Topic hyperlink highlighting is a truth state that varies. Topic hyperlink highlighting is true.

Topic hyperlink something is an activity on objects.
...
The style of object-word is hyperlink-bold-style. The style of direction-word is hyperlink-fixedwidth-style. The style of topic-word is hyperlink-italics-style.
...
Rule for printing a parser error when the latest parser error is not a verb I recognise error (this is the Hyperlink Interface not a verb I recognise rule) : show HI message for not-a-verb-I-recognise. [Acknowledge that the player may be trying to type a hyperlink, not just a verb.]
...
This documentation is copied directly from the extension "Keyword Interface" by Aaron Reed. The main modifications in the text are: "Keyword" replaced by "Hyperlink", "typed" replaced by "clicked", the color customization removed and so on.
...
The emphasized word will be the final word of the object's printed name. To change this, you can use the object's "keyword" property, which should be one of the other words in the printed name. You can also define an object as "hyperlinkless," in which case it will not be emphasized. Finally, you can manually mark an "[o]object[x]" with the o and x tags to emphasize it in object style.

	A telephone of Swedish origin with keyword "telephone" is in the Office. The description of the telephone is "You notice a faint [o]smudge[x] of lipstick on the receiver.". Some scattered cigarettes are a hyperlinkless thing in the Office.
