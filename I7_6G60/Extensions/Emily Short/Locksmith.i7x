Version 11 of Locksmith by Emily Short begins here.
...
Before locking an open thing (called the door ajar) with something (this is the closing doors before locking rule):
	...
	otherwise:
		say "(first closing [the door ajar])[command clarification break]";
...
Before opening a locked thing (called the sealed chest) (this is the unlocking before opening rule): 
	...
	otherwise:
		say "(first unlocking [the sealed chest])[command clarification break]";
...
The right second rule is listed instead of the can't unlock without the correct key rule in the check unlocking it with rulebook.
...
Carry out unlocking keylessly (this is the standard keylessly unlocking rule):
	if sequential action option is active, do nothing; otherwise say "(with [the matching key of the noun])[command clarification break]";
...
The right second rule is listed instead of the can't lock without the correct key rule in the check locking it with rulebook.
...
The can't put onto something being carried except keychains rule is listed instead
	of the can't put onto something being carried rule in the check putting it on rulebook.

This is the can't put onto something being carried except keychains rule:
	if the second noun is not a keychain:
		abide by the can't put onto something being carried rule.
