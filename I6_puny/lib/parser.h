...
[ YesOrNo;
    for (::) {
        ...
        if(parse -> 1 == 1) {
        	! one word reply
            if(parse --> 1 == 'oui' or 'o//') rtrue;
            if(parse --> 1 == 'non' or 'n//') rfalse;
        }
...
[ PronounNotice p_object;
	...
	if(p_object has animate) {
      if (p_object has pluralname) leur_obj = p_object;
      else lui_obj = p_object;
      if (p_object hasnt neuter) return; 
   }
   if (p_object has pluralname) les_obj = p_object;
   else {
      l_obj = p_object;
      if (p_object has female) la_obj = p_object;
      else le_obj = p_object; 
   }
];
...
!FRENCH
	while(p_parse_pointer --> 0 == 'le' or 'la' or 'les' or 'l^' or 'un' or 'une' or 'des' or 'de' or 'd^') { !TODO
		wn = wn + 1;
...
!FRENCH
	! skip 'the', 'all' etc !TODO de d^
	while(p_parse_pointer --> 0 == 'le' or 'la' or 'les' or 'l^' or 'un' or 'une' or 'des' or 'de' or 'd^' or ALL_WORD or EXCEPT_WORD1 or EXCEPT_WORD2) {
		if(p_parse_pointer --> 0 == ALL_WORD) {
...
!FRENCH
		if(_noun == 0) {
			...
			if(p_phase == PHASE2) {
				print "Je ne sais pas à quoi « ",(LanguagePronom) p_parse_pointer --> 0, " » se réfère.^";
				return -2;
			}
		} else if(TestScope(_noun) == false) {
			phase2_necessary = PHASE2_ERROR;
			if(p_phase == PHASE2) { !FIXME
				print "Vous ne pouvez pas voir « ",(LanguagePronom) p_parse_pointer --> 0, " » (", (name) _noun, ") pour l'instant.^";
				return -2;
			}
	 	}
		++wn;
...
!FRENCH
!TODO à/au/aux, etc. FIXME
!	print "Si vous voulez « "; !Je suppose que vous voul(i)ez dire / Pour / pouvez seulement
...
!					}
!				} else print (name) second; !_PrintObjName(second, FORM_DEF);
!			} else {
!				if(noun ~= 0) {
!					_noun = 0; ! avoid repeat (and we don't need _noun anymore)
!					print (name) noun; !_PrintObjName(noun, FORM_DEF);
...
!		}
!	}
!	print " » veuillez reformuler.^"; !Essayez à nouveau !Veuillez réessayer
!	rfalse;
!];
...
!FRENCH
				if(_noun ~= 'le' or 'la' or 'les' or 'l^' or 'un' or 'une' or 'des' or 'de' or 'd^') {
					if(noun==0)
						noun = _noun;
					else
						second = _noun;
...
!FRENCH
				if(parser_all_found || multiple_objects --> 0 > 1) print (name) noun, " : "; !FIXME
				if(inp1 > 1) PronounNotice(noun);
				PerformPreparedAction();
				++_score;
			}
