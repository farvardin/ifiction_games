...
!FRENCH
		...
		print _h%10, " h";
		if (status_field_2 > 0)
		print " ", status_field_2/10, status_field_2%10;
	}
];
...
[ PrintShortName o;
    if (o == 0) { print "nothing"; rtrue; } !FIXME
    switch (metaclass(o)) {
   ...
   if(p_obj hasnt proper && p_obj provides article) {
      if (p_obj.article ofclass Routine)
         ...
         if(p_form == FORM_CDEF)
            ...
            print (string) LanguageArticles-->(p_obj.article+1);
...
[ _PrintAfterEntry p_obj;
	if(p_obj has container && p_obj hasnt open) print " (fermé", (_es) p_obj, ")";
	if(p_obj has container && (p_obj has open || p_obj has transparent)) {
		if(child(p_obj) == nothing)
			print " (vide", (_s) p_obj, ")";
		else
			if(PrintContents(" (contenant ", p_obj)) print ")";
	}
	if(p_obj has supporter && child(p_obj) ~= nothing) {
		if(PrintContents(" (supportant ", p_obj)) print ")";
	}
!	if(p_obj has light && action == ##Inv) print " (providing light)";
#Ifndef OPTIONAL_NO_DARKNESS;
	if(p_obj has light) print " (éclairé", (_es) p_obj, ")";
#Endif;
	if(p_obj has worn && action == ##Inv) print " (porté", (_es) p_obj, ")";
];
...
!			(_obj has moved || _obj.initial == 0 || _obj notin parent(player))) {
	...
	if(_last_obj) {
		if(_printed_any_objects) print " et ";
		_PrintContentsPrintAnObj(_last_obj);
	}
...
Object selfobj "vous"
	with
      ...
      description "Ça a l'air bien !",
		...
		orders 0,
		number 0,
	has concealed animate proper transparent female;

!FRENCH
#Ifndef OPTIONAL_NO_DARKNESS;
Object thedark "L'obscurité"
	with
		initial 0,
		description "Vous êtes dans l'obscurité. Rien n'est visible.",
 		short_name 0,
		before 0,
		after 0;
#Endif;
...
#Endif;
	...
	for(::) {
		...
		verb_word = parse-->1;
		if(verb_word == 'recommencer') @restart;
		if(verb_word == 'charger') RestoreSub();
		if(AMUSING_PROVIDED == 0 && deadflag == 2 && verb_word == 'bonus') Amusing();
		if(verb_word == 'quitter') @quit;
#IfDef OPTIONAL_FULL_SCORE;
